 ...
 */
metadata {
	definition (name: "Aeon Multisensor DSB05", namespace: "kkdev", author: "SmartThings [kkmod]", runLocally: true, minHubCoreVersion: '000.017.0012', executeCommandsLocally: false) {
	...
	preferences {
		input "motionDelayTime", "number",
			title: "Motion Sensor Delay Time",
			description: "[Seconds] Amount of time the sensor will wait before reporting that motion has stopped. Enter a value 15-300.",
			range: "15..300",
			defaultValue: 240,
			required: false,
		...
		input "reportInterval", "number",
			...
			description: "[4 minute intervals] How often the device should report temperature, humidity, illuminance, and battery level. Enter a value 1 (4 minutes) - 360 (24 hours).",
			range: "1..360",
			defaultValue: 3,
			required: false,
			displayDuringSetup: false
...
def configure() {
	...
	delayBetween([
		...
		// send no-motion report xx seconds after motion stops
		zwave.configurationV1.configurationSet(parameterNumber: 3, size: 2, scaledConfigurationValue: motionDelayTime ?: 240).format(),
		...
		zwave.configurationV1.configurationSet(parameterNumber: 111, size: 4, scaledConfigurationValue: (reportInterval ?: 3) * 240).format()
	])
}
